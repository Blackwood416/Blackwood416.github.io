...
本文包含早期逐步调优记录，以及 2026-09-07 补充的泛化内核与预取同轮复测。后者的完整矩阵校验和官方库对照见第四部分对应章节；前文探索阶段的耗时与 VTune 记录保留原值，尚未逐项确认是否采用相同的队列、重置和计时流程，不能直接串成统一口径的性能曲线。本次更新澄清数据口径与结论边界，未新增硬件实测。

以下环境与流程说明适用于本次补充复测；早期记录以各节说明为准：
...
- **执行队列与数据依赖**：补充复测采用 SYCL 的 `property::queue::in_order` 队列，使同一队列内连续内核发射、中间重置与数据拷贝按提交顺序执行；每次评测末尾调用 `wait_and_throw()` 等待完成并报告异步错误。源码中的示例也已改用该队列属性，示例修改不代表早期各项耗时均已重测。
- **计时口径与重复测量**：使用单调时钟 `std::chrono::steady_clock` 包围连续迭代循环，预热后计算平均单次调用耗时，包含主机发射与设备执行。对于 beta 非零用例，还需确认 C 重置拷贝是否位于计时区间内；若包含，应称为“C 重置 + GEMM”的平均调用时间，TFLOPS 也属于该口径的有效吞吐。单个批次内多次迭代的均值不等于多个独立批次的统计结果，判断小幅收益时应补充交替测量的中位数与波动范围。
- **内存布局与转换成本**：本文后续手写内核接受满足对齐条件的预打包输入，报告的调用耗时不包含 A/B 布局转换。固定权重 B 可以复用一次打包的结果；运行时激活 A 通常需每次转换，除非上游算子直接生成目标布局。实际推理应另计 A 转换与必要传输成本，并与官方库统一输入契约；仅凭 B 可离线打包，不能忽略 A 的运行时成本。
- **正确性校验判据**：补充复测报告了与单精度 CPU 参考的全矩阵逐元素对拍，以及 beta 为零、旧 C 包含 NaN 的专用回归；形状、分母和绝对误差容差见泛化与预取章节。早期各阶段的 `C[0]` 冒烟输出不能替代全量对拍，也不代表每个历史内核均已完成相同的 NaN 回归。
...
本部分及后续 Joint Matrix 各节保留早期探索数据，用于说明优化尝试与当时的相对变化。最终补充复测采用的评测流程和数据单独列于第四部分，跨阶段耗时不直接作为同一口径的加速比。
...
本次采集中 L3 Bandwidth Bound 为 0.2%，不支持把该指标对应的带宽饱和视为主因，但不能单独排除访存延迟或其他存储层级的限制。手写内核的 Occupancy（58.4%）高于 oneDNN（28.7%），性能却更低，说明高占用率并不保证高效率。按表中计数，ALU 指令约为 oneDNN 的 4 倍、Send 约为 2.36 倍，因此后续优先尝试减少地址计算、数据重排与细碎消息；计数的采集范围与执行次数也应保持一致。
...
该阶段采集记录中，ALU1 指令数低于 oneDNN，Send 指令接近，同步屏障开销减少，Stall 从 46.8% 降至 40.7%。当时耗时为 0.0628 ~ 0.0646 ms，相对 oneMKL 约 82%、相对 oneDNN 约 87%。这些属于早期探索口径，不能与后文补充复测的约 63% ~ 68% 比例混用。
...
- 该阶段采集记录为平均执行耗时 59.7 µs、XMX Pipeline Active 20.7%（oneDNN 为 19.1%），XMX 指令发射速率约 86B/s（oneDNN 约 75B/s）；当时报告的比例为 oneDNN 的约 90%、oneMKL 的约 87%。这些是该阶段的历史记录，不作为本文最终复测的性能比例，也不能仅凭 XMX 活跃度推断有效计算效率更高。
...
这里的 `PASSED` 表示已报告的输入与判据下未检出错误，不等于逐位一致或覆盖所有数值分布。为便于独立复现，还需公开输入生成公式与范围、完整测试源码及编译参数，并记录实际最大绝对/相对误差和 NaN/Inf 检查方式。专用 NaN 回归也应列出所用 alpha，明确是否覆盖 `alpha!=1、beta=0` 的缩放路径；当前文字未给出这些细节，不能据此推定已经覆盖。
...
评测口径：本节内核均运行于 `property::queue::in_order` 队列，预热 20 轮，正式连续评测 100 轮取平均单次耗时；beta 非零的用例每轮计算前通过队列拷贝恢复初始矩阵 C，数值对拍覆盖全部 M×N 个输出元素。

**待核对的计时边界**：现有记录没有明确 C 重置拷贝是否包含在计时区间内。下表保留原有平均调用耗时和按此换算的有效 TFLOPS；确认前，不把 beta 非零行解释为纯 GEMM 设备执行时间。如果重置计入，应单列“重置 + GEMM”时间，并在独立采集中统计 GEMM event 的设备时间；即使重置不计时，也需考虑它对缓存状态和提交节奏的影响。

两项对比指标统一定义为：耗时降低率 $1 - t_{\text{prefetch}} / t_{\text{base}}$，吞吐提高率 $t_{\text{base}} / t_{\text{prefetch}} - 1$；正值表示预取加速，负值表示预取退化。base 指无显式预取的双缓冲基准；有效 TFLOPS 按 $2 \times M \times N \times K / (t_{\text{ms}} \times 10^9)$ 计算，受上文计时边界限制。
...
本轮 Deep、Wide、Small 的预取版本吞吐分别提高 9.5%、12.3%、25.5%；Baseline、Tall、Large、Huge 分别降低 4.8%、3.0%、2.4%、4.7%。这些结果说明预取在所测配置中的收益并不一致。

但存在一个尚未排除的混杂因素：**三个加速用例的 beta 全部非零，四个退化用例的 beta 全部为零**。形状变化同时伴随旧 C 读取路径、C 重置拷贝和缓存状态变化；alpha 也并不统一。因此，目前只能报告各配置的观测结果，不能直接将两组归类为“访存受限”与“计算饱和”，也不能反过来认定 beta 就是原因。预取减少加载等待、增加消息开销等机制仍需受控实验与相应计数器验证。

跨表比较也需要谨慎：上一节 Small 用例的无预取记录为 0.05495 ms，本节为 0.03672 ms，前者约高 50%。这不否定本节的同轮对照，但提示应回查代码版本、计时边界、运行条件与多批次波动，不能跨表挑选最好值，或仅凭一个批次就把 2% ~ 5% 的差异解释为稳定收益。

**待补的最小交叉实验（尚未报告结果）**：

| 固定形状 | alpha | beta | 每组比较 |
|---|---:|---:|---|
| Small：256×512×128 | 1 | 0 | 预取关闭 / PF_DIST=1 |
| Small：256×512×128 | 1 | 1 | 预取关闭 / PF_DIST=1 |
| Baseline：1024×1536×512 | 1 | 0 | 预取关闭 / PF_DIST=1 |
| Baseline：1024×1536×512 | 1 | 1 | 预取关闭 / PF_DIST=1 |

使用相同输入生成方式，分别采集 GEMM 设备时间与包含重置的调用时间；若要隔离重置对缓存的影响，再增加两种 beta 都执行相同重置操作的对照。各配置以交替顺序测量多个独立批次，报告中位数与波动范围，再判断收益更接近形状因素、写回路径还是基准流程的影响。

### 5. 关键调优经验与防御性准则

1. **预取距离（Prefetch Distance）的最佳选择**：
   - 按本节代码，循环第 b 轮的 `apb/brow` 指向第 b 块，`PF_DIST=1` 请求 b+1，随后 `load_block` 也加载 b+1；若要请求 b+2，应测试 `PF_DIST=2`。启动阶段与第 0 轮的重复预取已去重。本节数据只描述 PF_DIST=1 在这些形状与系数组合下的表现，最优距离仍需独立对照。
   - 增大距离可能改变缓存置换、消息队列压力与有效提前量，但本节没有列出 `PF_DIST>=3` 的同轮对照，不能据此断言收益通常递减；后续需在固定形状与系数下测量。
2. **数据类型与 DWORD 对齐规约**：
...
本文展示了从朴素实现逐步减少访存与辅助指令开销的过程。在预打包、对齐输入的 1024×1536×512 基准（alpha=1、beta=0）上，本次同轮复测的双缓冲版本为 0.07387 ms，预取版本为 0.07758 ms；按所选版本与参照库，达到 oneMKL/oneDNN 约 63% ~ 68% 的性能。在其他测试组合中，预取有加速也有退化，形状、系数与重置流程的影响尚待分离。可迁移的方法是明确契约、测量瓶颈和逐项验证。
...
早期探索记录展示了从标准 SYCL 分块、Joint Matrix 到 ESIMD 数据布局与流水线优化的过程；最终性能采用上文同轮复测作为参考。对 1024×1536×512、alpha=1、beta=0 的基准，双缓冲版本为 0.07387 ms，预取版本为 0.07758 ms，oneMKL/oneDNN 分别为约 0.0491/0.0504 ms。Deep 的 0.05919 ms 属于另一形状，不能与 Baseline 组成同一基准的性能区间；所有这些调用时间也均不包含 A/B 布局转换。

这段实践提供了三条可继续验证和迁移的方法：

1. **依据生成指令选择控制层次**：当分析发现布局重排或细碎消息开销时，可尝试 ESIMD 的显式向量布局与加载方式，并用实测判断收益；不能概括为显式控制在所有场景都优于高级抽象。
2. **用受控实验选择预取策略**：本轮 Small 配置记录了约 25.5% 的吞吐增益，也有多个配置退化。先排除 beta 与 C 重置的混杂影响，再结合缓存、消息和停顿指标解释原因，决定何时启用预取。
3. **将正确性与性能记录一起交付**：保留 beta 为零时不读取旧 C 的写回路径和预取边界守卫，同时公开输入、容差、实际误差与测量流程，使其他开发者能独立复现。

接下来优先补齐完整测试源码、编译命令与对应 commit、原始逐批次结果，以及输入生成规则、误差与 NaN 回归覆盖信息。完成计时边界核对和形状/beta 交叉实验后，再评估 A 布局转换与上游融合成本、非对齐尾块及实际推理负载。
